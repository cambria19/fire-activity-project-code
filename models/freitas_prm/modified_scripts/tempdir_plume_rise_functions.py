...
PLUME_ALONE_MODULE = os.path.abspath("./plume_alone_module")
vertical_profile = np.arange(50,20000,100) 
def setup_plume_sim(plume_namelist,plume_ID,thermo_profile, tmpdir):
    ...
    namelist_path = os.path.join(tmpdir, "plume_namelist")
    ...
    env_met_path = os.path.join(tmpdir, "env_met_input.dat")
    with open(os.devnull, 'w') as f, contextlib.redirect_stdout(f):
        with open(env_met_path, "w") as f_out:
        ...
        result = subprocess.run([PLUME_ALONE_MODULE], cwd=tmpdir, capture_output=True, text=True, check=True)
        detrain_profile, pth = compute_smoke_profile(result, tmpdir, vertical_profile)

    #Write the result 'log' and any error messages for the Freitas model simulation to a log file
    logfile_path = os.path.join(tmpdir, "plume_log")
    with open(os.devnull, 'w') as f, contextlib.redirect_stdout(f):
    ...
    if(len(result.stderr)==0):
        ...
        result_section = output[start_index:]
        print(result_section)
    else:
        print('Errors detected for '+plume_ID+'. Check log file for errors')

    return result, detrain_profile, pth



def compute_smoke_profile(result, tempdir, vertical_profile=None):
    ...
    thermo = pd.read_csv(f'{tempdir}/final_plume.dat', sep='\s+', header=None, usecols=[0, 1, 2, 11, 12, 13, 14])
    ...
    return detrain_profile, pth



def clean_up_run(plume_ID,output_directory,detrain_profile,write_plume_evo, tempdir):
    ...
    plume_profile = pd.read_csv(f'{tempdir}/final_plume.dat', sep='\s+', header=None)
    ...
    plume_profile.to_csv(f'{tempdir}/final_plume.dat', sep=' ', index=False, header=True, float_format='%.4f')
    ...
    detrain_profile.to_csv(f'{tempdir}/detrain_profile.dat', sep=' ', index=False, header=True, float_format='%.4f')
    ...
    os.system(f'mv {tempdir}/final_plume.dat '+full_outdir_name)
    os.system(f'mv {tempdir}/detrain_profile.dat '+full_outdir_name)
    os.system(f'mv {tempdir}/plume_log '+full_outdir_name)
    os.system(f'mv {tempdir}/plume_namelist '+full_outdir_name)
    os.system(f'mv {tempdir}/env_met_input.dat  '+full_outdir_name)
    if(write_plume_evo == True):
        os.system(f'mv {tempdir}/plumegen.dat '+full_outdir_name)
    else:
        os.system(f'rm {tempdir}/plumegen.dat')

    #Remove legacy output files. I think this is redundant with plumegen.dat anyways.
    os.system(f'rm {tempdir}/plumegen.gra')
